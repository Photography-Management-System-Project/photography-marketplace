...
export const AdminSupport = () => {
  ...
  useEffect(() => {
    javaApi.get('/admin/tickets').then(res => setTickets(res.data)).catch(console.error);
  }, []);
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {(selectedTicket.status !== 'Closed' && selectedTicket.status !== t('adminSupport.closed')) && (
                    <button 
                      onClick={async () => {
                        try {
                          const res = await javaApi.put(`/admin/tickets/${selectedTicket.id}`, { ...selectedTicket, status: 'Closed' });
